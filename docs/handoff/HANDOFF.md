...
| 2026-07-12 | P1-1 | Fable 5 (worktree от baseline, по старой версии карты) | E2EE-лейбл удалён из SettingsScreen целиком (новая карта предлагала замену на «Privacy & Offline» — по факту убран, честно в обоих вариантах); e2eeEnabled/e2eeKey → @deprecated; html2canvas убран из manualChunks; haptic={h} у EditorModal (совпало с D3); express/dotenv в deps уже отсутствовали. Ключи 'E2E Encryption & Blurring' в useTranslation.ts остались неиспользуемыми — вычистить в D4. Смержено в main вместе с fixes/p0. lint+build ✅ |

...
- [🔴 vite.config.ts: plugins-массив не закрыт `]`] -> tsc/build падали ещё до правок -> [🟢 закрыть массив перед resolve:]
- [🔴 SVG sizes:'any' в PWA-манифесте] -> Chrome не даёт установку, нужен числовой размер ≥144px -> [🟢 PNG 192+512 + отдельная maskable-запись]
- [🔴 два манифеста (manifest.json + VitePWA)] -> правки в мёртвый файл, он не подключён -> [🟢 единственный источник — блок manifest в vite.config.ts]
```
...
|---|---|---|---|
| 2026-07-15 | P1-1 | Fable 5 | e2ee deprecated в constants; html2canvas из manualChunks; EditorModal haptic={h}; попутно починен незакрытый plugins-массив в vite.config.ts. Ворота ✅ |
| 2026-07-15 | PWA-иконки (вне карт) | Fable 5 | pwa-192/512 + maskable + apple-touch-icon из icon.svg (sharp); манифест на PNG; мёртвый manifest.json удалён. Установка в Chrome подтверждена юзером ✅ |

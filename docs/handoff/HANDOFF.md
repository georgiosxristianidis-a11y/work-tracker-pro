...
- [🔴 Битый vite.config.ts в baseline (не закрыт массив plugins)] -> build падал, ворота непроходимы -> [🟢 запускать npm run build сразу после baseline, а не только после правок]
...
|---|---|---|---|
| 2026-07-12 | P1-1 | Fable 5 | Убран E2EE-лейбл из SettingsScreen; e2eeEnabled/e2eeKey помечены @deprecated; html2canvas убран из manualChunks; EditorModal получает `h` (вибрация уважает настройку); express/dotenv в deps уже отсутствовали. Попутно починен битый синтаксис vite.config.ts (не закрыт plugins[]) — иначе ворота не проходили. lint+build ✅ |
